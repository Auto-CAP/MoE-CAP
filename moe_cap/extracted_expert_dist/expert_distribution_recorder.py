...
class _ExpertDistributionRecorderReal(ExpertDistributionRecorder):
    ...
    def stop_record(self) -> None:
        """Stop recording."""
        if not self._recording:
            logger.warning("Expert distribution recorder not currently recording")
            return
        self._recording = False
...
class _PerTokenGatherer(_SinglePassGatherer):
    ...
    def __init__(
        ...
        self._device = torch.device(device) if isinstance(device, str) else torch.device(device)
    ...
    def on_select_experts(self, layer_idx: int, topk_ids: torch.Tensor) -> None:
        ...
        stored = self._layer_records[layer_idx]
        topk_ids = topk_ids.detach().clone()
        if stored is None:
            self._layer_records[layer_idx] = topk_ids
        else:
            self._layer_records[layer_idx] = torch.cat((stored, topk_ids), dim=0)

        updated = self._layer_records[layer_idx]
    ...
    def _reset_storage(self) -> None:
        self._layer_records: list[torch.Tensor | None] = [None] * self._num_layers
        self._max_tokens = 0
        self._max_topk = 0
...
class _StatApproxGatherer(_SinglePassGatherer):
    ...
    def on_expert_dispatch(
        ...
        num_tokens_per_expert = num_tokens_per_expert.to(
            device=self._expert_count.device,
            dtype=self._expert_count.dtype
        )
        # Accumulate instead of replace
        self._expert_count[layer_idx] += num_tokens_per_expert
...
class _Accumulator(ABC):
    ...
    def append(self, forward_pass_id: int, single_pass_data: Dict[str, Any]) -> None:
...
class _PerTokenAccumulator(_Accumulator):
    ...
    def dump(self, output_path: Optional[str] = None) -> Dict[str, Any]:
        ...
        if output_path:
            logger.info(f"Saving expert distribution recording to {output_path}")
    ...
    def append(self, forward_pass_id: int, single_pass_data: Dict[str, Any]) -> None:
        ...
        if "expert_count" in single_pass_data:
            ...
            if total_sum > 0:
                ...
                activated_per_layer = activated_experts.sum(dim=1)  # Sum across experts per layer
                total_activated = activated_per_layer.sum().item()  # Total activated experts
                ...
                self._pass_records.append({
                    ...
                    "avg_activated_per_layer": round(avg_activated_per_layer, 3),
                    "activated_per_layer": activated_per_layer.tolist(),
